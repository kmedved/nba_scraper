...
class PbP:
    ...
    def __init__(self, pbp_df):
        ...
        if "scoremargin" in self.df.columns:
            self.df["scoremargin"] = self.df["scoremargin"].astype(str)
        else:
            self.df["scoremargin"] = ""

        # Final FT of a trip (new structured path; fallback to legacy text)
        if "ft_n" in self.df.columns and "ft_m" in self.df.columns:
            self.df["is_last_ft"] = (
                (self.df["event_type_de"] == "free-throw")
                & (
                    self.df["ft_n"].fillna(-1).astype(int)
                    == self.df["ft_m"].fillna(-2).astype(int)
                )
            ).astype(int)
        else:
            hd_series = self.df.get("homedescription")
            vd_series = self.df.get("visitordescription")
            if hd_series is None:
                hd = pd.Series([""] * len(self.df))
            else:
                hd = hd_series.astype(str)
            if vd_series is None:
                vd = pd.Series([""] * len(self.df))
            else:
                vd = vd_series.astype(str)
            self.df["is_last_ft"] = (
                (self.df["event_type_de"] == "free-throw")
                & (
                    hd.str.contains(r"Free Throw\s+2 of 2", regex=True, na=False)
                    | hd.str.contains(r"Free Throw\s+3 of 3", regex=True, na=False)
                    | vd.str.contains(r"Free Throw\s+2 of 2", regex=True, na=False)
                    | vd.str.contains(r"Free Throw\s+3 of 3", regex=True, na=False)
                )
            ).astype(int)
